...
// 	err := r.db.WithContext(ctx).Where("user_id = ? AND status = 'active'", userID).First(&subscription).Error
// 	if err != nil {
// 		r.log.Error("failed to get user subscription",
// 			logger.ErrorField(err),
...
// 	if err != nil {
// 		r.log.Error("failed to get user payments",
// 			logger.ErrorField(err),
// 			logger.String("userID", userID))
// 		return nil, err
// 	}
